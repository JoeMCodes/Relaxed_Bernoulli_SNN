...
os.chdir('Experiment_DVSGesture/det_models')
...
with open('config_Det.json', 'r') as f:
    config = json.load(f)
...
def train_and_test(device, train_loader, test_loader, beta, temp, num_epochs, num_steps, batch_size, lr, threshold): ### numsteps now defined in train/ testloader
    ...
    torch.save(net.state_dict(), f'deterministic_model/Models/MSE_count_beta{beta}_temp{temp}_num_epochs{num_epochs}_num_steps{num_steps}_batch_size{batch_size}_lr{lr}_thr_{threshold}.pt') ## Save model
    ...
    with torch.no_grad():
        ...
        plt.savefig(f'deterministic_model/Results/MSE_count_beta{beta}_temp{temp}_num_epochs{num_epochs}_num_steps{num_steps}_batch_size{batch_size}_lr{lr}_thr_{threshold}.png')
        plt.close()

        # Save all results to a file
        with open(f'deterministic_model/Results/MSE_count_beta{beta}_temp{temp}_num_epochs{num_epochs}_num_steps{num_steps}_batch_size{batch_size}_lr{lr}_thr_{threshold}.pkl', 'wb') as f:
