...
with open('config_ASSnet.json', 'r') as f:
    config = json.load(f)
...
def trainer(net, train_loader, test_loader, device, num_epochs, lr, batch_size, temp_sched):### numsteps now defined in train/ testloader
    loss = nn.MSELoss()
    ...
    for epoch in range(num_epochs):
        ...
        for data, label in iter(train_loader):
            ...
            with torch.cuda.amp.autocast():
                ...
                loss_val = temp_sched[epoch] * loss(output, targets)
...
def train_and_test(device, train_loader, test_loader, beta, temp_sched, num_epochs, num_steps, batch_size, lr, threshold): ### numsteps now defined in train/ testloader
    print(f'Started training for beta {beta} time {num_steps} _thr_{threshold}_lr{lr}', flush=True)
    set_seed(42)

    net = Net(beta = beta, temp = 0.1, threshold=threshold, device = device).to(device) ## Create net

    train_hist, test_hist, test_acc_hist = trainer(net, train_loader, test_loader, num_epochs = num_epochs, device = device, batch_size=batch_size, lr = lr, temp_sched = temp_sched) # train net

    torch.save(net.state_dict(), f'ASSnet_tests/Models/MSE_count_beta{beta}_num_epochs{num_epochs}_num_steps{num_steps}_batch_size{batch_size}_lr{lr}_thr_{threshold}.pt') ## Save model
    ...
    with torch.no_grad():
        ...
        plt.savefig(f'ASSnet_tests/Results/MSE_count_beta{beta}_num_epochs{num_epochs}_num_steps{num_steps}_batch_size{batch_size}_lr{lr}_thr_{threshold}.png')
        plt.close()

        # Save all results to a file
        with open(f'ASSnet_tests/Results/MSE_count_beta{beta}_num_epochs{num_epochs}_num_steps{num_steps}_batch_size{batch_size}_lr{lr}_thr_{threshold}.pkl', 'wb') as f:
...
for timestep in timesteps:
    ...
    for beta in betas:
        for threshold in thresholds:
            for lr in lrs:
                big_t = time.time()
                train_and_test(device=device, train_loader=trainloader, test_loader=testloader, beta=beta, temp_sched=temp_sched, num_epochs=num_epochs, num_steps=60, batch_size=batch_size, lr=lr, threshold=threshold)
